...
class BannerController extends BaseController
{
    public function index($layout)
    {
        ...
        $filter->add('customer_vip_discount_id', '活动ID', 'text')
            ->scope(function ($query, $value) {
                return $value !== null ? $query->where('b.customer_vip_discount_id', $value) : $query;
            });
...
//        $grid->add('cover','覆盖人数',false);
        ...
        $grid->row(function ($row) use ($grid) {
            ...
            if (empty($result)) {
                $row->cell('status')->value = '异常，请检查时间!';
    ...
    public function anyEdit($layout)
    {
        ...
        $layoutData = Platv4Layout::where('alias', $layout)->first();
        $layoutData && Input::offsetSet('layout_id', $layoutData->id);
        ...
        if ($bindId) {
            ...
            $this->route = 'banners/' . $layout . '?search=1&customer_vip_discount_id=' . $bindId;
        }

        $title = 'banner信息';
        !empty($layoutData->name) && $title .= $layoutData->name;
        $edit->label($title);
