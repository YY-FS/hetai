...
class UserPaymentController extends BaseController
{
    public function index()
    {
        ...
        $filter->add('order_type', '支付类型', 'select')->options(['' => '全部类型'] + Platv4UserPayment::$orderTypeText)
            ->scope(function ($query, $value) {
        ...
        $filter->add('pay_source', '设备', 'select')->options(['' => '全部设备'] + Platv4UserPayment::$sourceText)
            ->scope(function ($query, $value) {
                return $value ? $query->where('t.name', $value) : $query;
            });
        $filter->add('bundle_id', 'BundleId', 'text')
            ->scope(function ($query, $value) {
